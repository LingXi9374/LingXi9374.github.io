...
export const fontsList: FontDefinition[] = [
	...
	// 3. 在 fontConfig.selected 或区域字段中引用 cssVariable
	{
		...
		cssVariable: "--font-lxgw-wenkai-mono-gb-screen",
		provider: "local",
		options: {
			variants: [
				{
					src: ["./public/assets/fonts/LXGWWenKaiMonoGBScreen.ttf"],
				},
			],
		},
		fallbacks: ["sans-serif"],
	...
	{
		name: "Maple Mono NF CN Medium",
		cssVariable: "--font-maple-mono-nf-cn",
		provider: "local",
		options: {
			variants: [
				{
					src: ["./public/assets/fonts/MapleMono-NF-CN-Medium.ttf"],
				},
			],
		},
		fallbacks: [
...
export const fontConfig: FontSelectionConfig = {
	...
	selected: ["--font-lxgw-wenkai-mono-gb-screen"],

	// 各区域独立字体设置（填写上方 fonts 中的 cssVariable，留空则使用全局 selected 字体）
	// 例如：bannerTitleFont: "--font-inter", 表示主页横幅主标题使用 Inter 字体
	// 主页横幅主标题字体
	bannerTitleFont: "--font-lxgw-wenkai-mono-gb-screen",
	// 主页横幅副标题字体
	bannerSubtitleFont: "--font-lxgw-wenkai-mono-gb-screen",
	// 导航栏标题字体
	navbarTitleFont: "--font-lxgw-wenkai-mono-gb-screen",
	// 代码块字体（用于代码高亮和等宽字体场景）
	codeFont: "--font-maple-mono-nf-cn",

	// 本地字体子集化配置（构建时由 scripts/subset-fonts.ts 处理）
	// key 为 fonts 数组中对应的 cssVariable，value 为子集化选项
	subsetFonts: {
		"--font-lxgw-wenkai-mono-gb-screen": {
			// 额外包含的字符
			extraChars: "",
		},
	},
